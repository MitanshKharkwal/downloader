...
3. BUG 2 (retry contract broken/dead code)
4. BUG 4 (latent crash on state save)
5. BUG 5 (silent stuck download edge case)
6. BUG 6 (verify against libtorrent docs, low impact)

After fixing, run the existing suite in `tests/` (especially `tests/test_ipc_server.py`, `tests/resume_test.py`, `tests/retry_test.py`, `tests/rate_limit_test.py`) plus any new tests added for BUGS 3 and 5, and confirm nothing regresses.
