...
class DownloadRow(BoxLayout):
    ...
    def __init__(self, manager: DownloadManager, task_id: str, **kwargs):
        super().__init__(orientation="vertical", size_hint_y=None, height=dp(96), padding=dp(8), spacing=dp(4), **kwargs)
        self.manager = manager
        self.task_id = task_id

        top = BoxLayout(size_hint_y=None, height=dp(32), spacing=dp(8))
        self.name_label = Label(text="", halign="left", valign="middle", size_hint_x=0.5, font_size=sp(15))
        ...
        self.detail_label = Label(text="", size_hint_y=None, height=dp(22), font_size=sp(12))
    ...
    def _open_options(self, btn) -> None:
        ...
        if task.status == DownloadStatus.ERROR:
            ...
            btn_retry.bind(on_release=lambda x: [dropdown.dismiss(), self.manager.retry(self.task_id)])
            dropdown.add_widget(btn_retry)
        elif task.status in (DownloadStatus.DOWNLOADING, DownloadStatus.QUEUED, DownloadStatus.CONNECTING):
            btn_pause = Button(text="Pause", size_hint_y=None, height=44)
            btn_pause.bind(on_release=lambda x: [dropdown.dismiss(), self.manager.pause(self.task_id)])
            ...
            btn_resume.bind(on_release=lambda x: [dropdown.dismiss(), self.manager.resume(self.task_id)])
            dropdown.add_widget(btn_resume)

        # Priority Options
        if task.status not in (DownloadStatus.COMPLETED, DownloadStatus.CANCELED):
            for label, p in [("High Priority", Priority.HIGH), ("Normal Priority", Priority.NORMAL), ("Low Priority", Priority.LOW)]:
                if task.priority == p: continue
                btn_p = Button(text=label, size_hint_y=None, height=44)
                btn_p.bind(on_release=lambda x, p=p: [dropdown.dismiss(), self.manager.set_priority(self.task_id, p)])
                dropdown.add_widget(btn_p)

        # Cancel
        if task.status not in (DownloadStatus.COMPLETED, DownloadStatus.CANCELED):
            btn_cancel = Button(text="Cancel", size_hint_y=None, height=44)
            btn_cancel.bind(on_release=lambda x: [dropdown.dismiss(), self._on_cancel()])
    ...
    def _on_cancel(self, *_args) -> None:
        ...
        if task.downloaded_bytes > _CANCEL_CONFIRM_BYTES or task.progress() > _CANCEL_CONFIRM_PCT:
            from kivy.uix.popup import Popup
            from kivy.uix.label import Label
            from kivy.uix.boxlayout import BoxLayout
            content = BoxLayout(orientation="vertical", padding=10, spacing=10)
            content.add_widget(Label(text="Are you sure you want to cancel this download?\nPartial data will be deleted."))
