...
EXT_PATH = os.path.dirname(_THIS_DIR)
...
with sync_playwright() as p:
    ...
    assert sw is not None, (
        "extension service worker never registered -- manifest/background.js problem"
    )
    ...
    assert page.url == url_before, (
        "clicking the magnet link should NOT navigate the page"
    )
    ...
    assert (
        last_result["lastResult"]["ok"] is False
    )  # expected: no native host in this test profile
    ...
    print(
        "native messaging itself correctly reports 'not connected' since no host is registered in this test profile)"
    )
